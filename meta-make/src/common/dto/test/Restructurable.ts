...
describe('Restructurable class, when restructured', () => {
  it("should remain untouched if it's a primitive", () => {
    const sourceNumber = 25
    const clonedNumber = structuredClone(sourceNumber)
    const restructuredNumber = Restructurable.restructure(clonedNumber)

    assert.strictEqual(sourceNumber, restructuredNumber)
    assert.ok(typeof restructuredNumber === 'number')
  })

  it("should remain untouched if it's a built-in", () => {
    const sourceArray = [1, 2, 3]
    const clonedArray = structuredClone(sourceArray)
    const restructuredArray = Restructurable.restructure(clonedArray)

    assert.deepEqual(restructuredArray, sourceArray)
    assert.ok(restructuredArray instanceof Array)
  })
  ...
  it('should restructure any nested restructurables as well', () => {
    const list = new ArrayList([
      new ArrayList([new Integer(3), new Integer(5)]),
      new Integer(15),
      75
    ])
