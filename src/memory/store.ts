import N3, {
    BlankNode, DataFactory, NamedNode, OTerm, Quad, Quad_Graph, Quad_Object, Quad_Predicate, Quad_Subject, Store, Term,
} from 'n3';
import { getScopedLogger, ScopedLogger } from '../logger.js';
import { Configuration, Processor } from '../processor/processor.js';
import { getCompactName, isDefaultGraph } from './utils.js';
import {
    hasObject, hasPredicate, hasSubject, isA, prefixToNamespace, ProvenanceEntity, Statement, wasDerivedFrom,
} from './vocabulary.js';
...
export class MetaStore {
    ...
    public static getStore(identifier?: string): MetaStore {
        if (!identifier) return this.defaultStore;
     ...
     */
    public one(subject?: Quad_Subject | null, predicate?: Quad_Predicate | null, object?: Quad_Object | null, graph?: Quad_Graph | null): Quad {
        const match = this.store.match(subject, predicate, object, graph);

        if (match.size !== 1) throw new Error(`Failed to match single statement for 
     ...
     */
    public oneOrNone(subject?: Quad_Subject | null, predicate?: Quad_Predicate | null, object?: Quad_Object | null, graph?: Quad_Graph | null): Quad | null {
        const match = this.store.match(subject, predicate, object, graph);

        if (match.size === 0) return null;

        if (match.size > 1) throw new Error(`Failed to match single statement for ${subject} ${predicate} ${object} ${graph}. Found ${match.size}`);
     ...
     */
    public oneOrDefault(subject?: Quad_Subject | null, predicate?: Quad_Predicate | null, object?: Quad_Object | null, graph?: Quad_Graph | null): Quad | null {
        const match = this.store.match(subject, predicate, object, graph);
     ...
     */
    public all(subject?: Quad_Subject | null, predicate?: Quad_Predicate | null, object?: Quad_Object | null, graph?: Quad_Graph | null): Quad[] {
        return [...this.store.getQuads(subject, predicate, object, graph)];
    }

    public getProvenanced(subject: Quad_Subject, predicate: Quad_Predicate, object: Quad_Object, graph: Quad_Graph): Quad_Subject | null {
    ...
    public add(subject: Quad_Subject, predicate: Quad_Predicate, object: Quad_Object, graph?: Quad_Graph | null, origin?: Quad | null, author?: Processor<Configuration>, details?: [Term, Term][]): void {
        this.logger.debug(`+ QUAD : ${getCompactName(subject)} ${getCompactName(predicate)} ${getCompactName(object)} ${getCompactName(graph) ?? ''}`, origin || author || details ? {
            data: {
                origin,
                author, ...details,
            },
    ...
    private getProvenanceGraph(graph: Quad_Graph | null): Term {
        ...
        let provenanceGraph = this
            .oneOrNone(graph, hasProvenanceGraph, null, ProvenanceMappingGraph)?.object;
