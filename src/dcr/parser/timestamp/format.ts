import { hasDuplicates } from '../../utils/array.js';
import { must } from '../../utils/logic.js';
import { TimeOfDay } from '../../utils/time.js';
import {
    getTokenDetailsByLabel, TimestampCategory, TimestampLabelToToken, TimestampSpecificity, TimestampTokenDetails,
} from './tokens.js';
import { Timestamp, TimestampType } from './useType.js';
...
export function determineTypeFromFormatting(format: string[]): TimestampType {
    ...
    const timeCategories = [TimestampCategory.Hours, TimestampCategory.Minutes, TimestampCategory.Seconds, TimestampCategory.Milliseconds];

    const dateCategories = [TimestampCategory.Years, TimestampCategory.Months, TimestampCategory.Days, TimestampCategory.Era];

    let containsDate = false;
    if (tokenCategories.some(category => dateCategories.includes(category))) containsDate = true;

    let containsTime = false;
    if (tokenCategories.some(category => timeCategories.includes(category))) containsTime = true;

    if (containsTime && containsDate) return TimestampType.DATETIME;

    if (containsTime) return TimestampType.TIMEOFDAY;

    if (containsDate) return TimestampType.DATE;
...
function validateTimestampFormat(format: string[]): boolean {
    ...
    if (hasDuplicates(categories)) return false;

    if (!enforceCustomRules(format)) return false;
...
function enforceCustomRules(format: string[]): boolean {
    ...
    function has(...categories: TimestampCategory[]) {
        return function (format: string[]) {
            return categories.every(category => format.some(label => getTokenDetailsByLabel(label).category === category));
        };
    }
    ...
    function areInOrderIfExist(cat1: TimestampCategory | TimestampCategory[], cat2: TimestampCategory | TimestampCategory[]): (format: string[]) => boolean {
        ...
        return must.imply(has(cat1, cat2), areInOrder(cat1, cat2));
    }
    ...
    const compliesInclusion = (format: string[]) => {
        const categoriesInOrder = [TimestampCategory.Years, TimestampCategory.Months, TimestampCategory.Days, TimestampCategory.Hours, TimestampCategory.Minutes, TimestampCategory.Seconds, TimestampCategory.Milliseconds];
    ...
    const compliesContinuity = must.all(must.imply(has(TimestampCategory.Milliseconds), has(TimestampCategory.Seconds)), must.imply(has(TimestampCategory.Meridiem), has(TimestampCategory.Hours)), must.imply(has(TimestampCategory.Era), has(TimestampCategory.Years)));

    // Despite various nature of datetime formats, some things are constant
    const compliesSuccession = must.all(// time tokens are in direct order (hours, minutes, seconds, milliseconds)
        areInOrderIfExist(TimestampCategory.Hours, TimestampCategory.Minutes), areInOrderIfExist(TimestampCategory.Minutes, TimestampCategory.Seconds), areInOrderIfExist(TimestampCategory.Seconds, TimestampCategory.Milliseconds), areInOrderIfExist(TimestampCategory.Hours, TimestampCategory.Meridiem),
        ...
        areInOrderIfExist([TimestampCategory.Years, TimestampCategory.Months, TimestampCategory.Days, TimestampCategory.Era, TimestampCategory.DayOfWeek], [TimestampCategory.Hours, TimestampCategory.Minutes, TimestampCategory.Seconds, TimestampCategory.Milliseconds, TimestampCategory.Meridiem]),

        must.imply(has(TimestampCategory.Years, TimestampCategory.Months, TimestampCategory.Days), must.any(// UTC format
            areInOrder(TimestampCategory.Years, TimestampCategory.Months, TimestampCategory.Days), // common format
            areInOrder(TimestampCategory.Days, TimestampCategory.Months, TimestampCategory.Years), // american format
            areInOrder(TimestampCategory.Months, TimestampCategory.Days, TimestampCategory.Years))));

    const compliesNumericSeparation = (format) => {
        const numericSpecificities = [TimestampSpecificity.NumericLong, TimestampSpecificity.NumericMedium, TimestampSpecificity.NumericShort];
