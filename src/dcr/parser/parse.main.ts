...
import { recognizeEnums } from './enum/recognize.js';
import { EnumUseType } from './enum/useType.js';
import { recognizeNumbers } from './number/recognize.js';

import { enumConstants, numberConstants, timestampConstants } from './parse.constants.js';
import { recognizeStrings } from './string/recognize.js';
import { recognizeTimestamps } from './timestamp/recognize.js';
import { UseType, UseTypeArgs } from './useType.js';
...
const defaultRecognizerArgs: RecognizerArgs = {
    skipConstants: false, sizeHardLimit: 50, sampleSize: 500,
};
...
export function determineType(data: string[], args: RecognizerArgs): UseType<unknown>[] {
    ...
    logger.info(`determineType for ${args.label} finished`, {
        args,
        useTypes: gatheredUseTypes.map(ut => ut.toString()),
    });
...
function preprocessHardLimitSize(source, args) {
    if (!args.sizeHardLimit) return true;
    const withinSizeLimit = source.every(sample => sample.length < args.sizeHardLimit);
    if (!withinSizeLimit) args.limitExceeded = true;
